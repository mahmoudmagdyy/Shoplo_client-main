...
class SearchMapSuggestionsDataProvider {
  ...
  Future<AppResponse> getGoogleStores(query) async {
    late final AppResponse response;
    await DioHelper.getData(
      url: MapConfiguration.nearBySearchBaseUrl,
      query: {
        ...query,
        'key': MapConfiguration.apiKeyMaps,
      },
      ...
      (value) {
        debugPrint('getGoogleStores VALUE: $value', wrapWidth: 1024);
        debugPrint(' getGoogleStores VALUE in get: ${value.data['results']}',
            wrapWidth: 1024);
        response = AppResponse.fromJson({'data': value.data['results']});
        response.pageToken = value.data['next_page_token'];
      },
    ).catchError(
      (error) {
        debugPrint('getGoogleStores ERROR: $error}', wrapWidth: 1024);
        debugPrint(
            'getPlaceDetails ================================ ${error?.message}');
        if (error?.response != null) {
          response = AppResponse.withErrorResponse(error.response);
