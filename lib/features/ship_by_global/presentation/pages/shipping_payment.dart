...
import 'package:shoplo_client/features/ship_by_global/presentation/cubit/ship_by_global_cubit.dart';
...
class ShippingPaymentScreen extends HookWidget {
                      ...
                      builder: (context, state) {
                        ...
                        return AppList(
                          ...
                          itemBuilder: (context, index) {
                            if (cubit.paymentMethods[index].type == 'cash_on_delivery') {
                              return Container();
                            }
          ...
          Padding(
              ...
              child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                ...
                BlocProvider(
                  ...
                  child: BlocConsumer<ShipByGlobalCubit, ShipByGlobalState>(
                    listener: (context, state) {
                      ...
                      if (state is ShipByGlobalErrorState) {
                        if (state.errors.isNotEmpty) {
                          AppToast.showToastSuccess(state.errors);
                        }
                      }
                      if (state is ShipByGlobalPaymentState) {
                        AppToast.showToastSuccess(context.tr.send_bank_transfer);
                        Navigator.pop(context);
                      }
                    },
                    builder: (context, state) {
                      return AppButton(
                        loading: state is ShipByGlobalLoadingState,
                        width: context.width * .7,
                        onPressed: () {
                          if (selectedPaymentMethod.value.id == -1) {
                            AppToast.showToastError(context.tr.select_payment_method);
                            return;
                          }
                          BlocProvider.of<ShipByGlobalCubit>(context).payment(
                            int.tryParse(orderId) ?? 0,
                            {
                              ...
                              "use_wallet": isUseWallet.value ? 1 : 0,
                            },
                          );
                        },
